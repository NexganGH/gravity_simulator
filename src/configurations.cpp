...
std::vector<std::shared_ptr<Configuration>> getConfigurations(
  ...
  CreateSolarSystem(list, window);
  CreateEarthAndSun(list, window);
  CreateStarCollision(list, window);
  CreateStableOrbitTwoPlanets(list, window);
  CreateStableOrbitTwoPlanets2(list, window);
  CreateThreeBodies(list, window);
  CreateBinaryStars(list, window);
  CreateCollapsingBinaryStars(list, window);

  return list;
}

void CreateSolarSystem(std::vector<std::shared_ptr<Configuration>> list,
                       sf::RenderWindow& window) {
...
const float NEPTUNE_MASS = 17 * EARTH_MASS;
  ...
  auto SolarSystem =
      new Configuration("Solar System", 3 * MARS_AFELIO, 1000000, window);
  ...
  list.push_back(std::unique_ptr<Configuration>(SolarSystem));
}

void CreateEarthAndSun(std::vector<std::shared_ptr<Configuration>> list,
                       sf::RenderWindow& window) {
  const float EARTH_SUN_DISTANCE = 149597870700;
  const float EARTH_SPEED = 30.24E3;
  const float EARTH_MASS = 5.9722E24;
  ...
  auto EarthAndSun =
      new Configuration("Earth & Sun", 3 * EARTH_SUN_DISTANCE, 1000000, window);
...
void CreateStarCollision(std::vector<std::shared_ptr<Configuration>> list,
  ...
  auto StarCollision =
      new Configuration("Star Collision", 3 * EARTH_SUN_DISTANCE, 1000000, window);
...
void CreateStableOrbitTwoPlanets(std::vector<std::shared_ptr<Configuration>> list,
                         sf::RenderWindow& window) {
...
void CreateStableOrbitTwoPlanets2(std::vector<std::shared_ptr<Configuration>> list,
                         sf::RenderWindow& window) {
...
void CreateThreeBodies(std::vector<std::shared_ptr<Configuration>> list,
                         sf::RenderWindow& window) {
auto ThreeBodies =
      new Configuration("Three Bodies", 2000, 1000000, window);
...
list.push_back(std::unique_ptr<Configuration>(ThreeBodies));
}

void CreateBinaryStars(std::vector<std::shared_ptr<Configuration>> list,
                         sf::RenderWindow& window) {
auto BinaryStars =
      new Configuration("Binary Stars", 2000, 1000000, window);
...
void CreateCollapsingBinaryStars(std::vector<std::shared_ptr<Configuration>> list,
                         sf::RenderWindow& window) {
