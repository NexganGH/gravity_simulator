...
double distance(std::unique_ptr<Body> &b1, std::unique_ptr<Body> &b2){
   return std::sqrt(std::pow(b1->getPosition().x-b2->getPosition().x,2)+std::pow(b1->getPosition().x-b2->getPosition().x, 2));
}
...
//implementazione eulero
     ...
     // auto newPosition = Vector { pos.x + vel.x * dt + 0.5*acc.x*dt*dt+0.1666667*b1->getAccDer().x*dt*dt*dt, pos.y + vel.y * dt + 0.5*acc.y *dt* dt+0.1666667*b1->getAccDer().y*dt*dt*dt};
     auto newPosition = Vector { pos.x + vel.x * dt + 0.5*acc.x*dt*dt, pos.y + vel.y * dt + 0.5*acc.y *dt* dt};
